...
import { Card, CardBody, CardFooter, Image, Skeleton, Badge } from "@heroui/react";
          ...
          birthdayData
            ...
            .map((member, index) => (
              <Badge
                ...
                placement="top-right"
                size="lg"
                isInvisible={!isBirthdayToday(member.birthdate)}
                classNames={{
                  badge: "animate-pulse"
                }}
              >
                <Card 
                  isPressable 
                  shadow="sm" 
                  className={isBirthdayToday(member.birthdate) ? "ring-2 ring-warning-400 ring-offset-2" : ""}
                ...
                >
                  ...
                  <CardFooter className="text-small justify-between flex-col items-start">
                    <b className={isBirthdayToday(member.birthdate) ? "text-warning-600" : ""}>
                      {member.name}
                    </b>
                    <div className="w-full flex justify-between items-center mt-1">
                      <p className="text-default-500">{formatBirthdate(member.birthdate)}</p>
                      <p className={isBirthdayToday(member.birthdate) ? "text-warning-600 font-bold" : "text-primary-500"}>
